  ...
  try {
    ...
    for (let batchStart = 0; batchStart < totalPhotos; batchStart += batchSize) {
      ...
      const batchPromises = batch.map(async (photo: any, batchIndex: number) => {
        ...
        console.log(`🔍 Photo ${globalIndex} object:`, {
          alt: photo.alt,
        ...
        try {
          ...
          // Use original filename from alt field (which contains the actual filename like DSC05430.JPG)
          let originalFilename = photo.alt || '';
          console.log(`🔍 Photo ${globalIndex}: alt="${originalFilename}"`);
          
          if (!originalFilename) {
            // Fallback: try to extract from Google Drive URL
            try {
              if (photo.url.includes('/file/d/')) {
                const urlParts = photo.url.split('/');
                const lastPart = urlParts[urlParts.length - 1];
                if (lastPart && lastPart.includes('.')) {
                  originalFilename = decodeURIComponent(lastPart.split('?')[0]);
                }
              }
            } catch (extractError) {
              console.log('Could not extract filename from URL, using fallback');
            }
          }
          
          console.log(`📁 Using filename: "${originalFilename}"`);
          
          // Generate filename - use original if available, otherwise fallback
          ...
          let filename;
          if (originalFilename) {
            // Use original filename directly - it already has the correct extension
