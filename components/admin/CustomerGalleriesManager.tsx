      ...
      if (response.ok) {
        ...
        const galleriesWithStatus = await Promise.all(
          data.map(async (gallery: CustomerGallery) => {
            ...
            // Fallback to faceIndexing if indexingStatus API fails
            return {
              ...gallery,
              indexingStatus: gallery.faceIndexing ? {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (homeServerResponse.ok) {
          ...
          // Force refresh immediately and again after 2 seconds to show progress
          await fetchIndexingStatuses();
          setTimeout(() => fetchIndexingStatuses(), 2000);
        } else {
